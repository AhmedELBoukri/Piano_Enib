# -*- coding: utf-8 -*-
# inspired by Fabrice Sincère from audio.py 
...
def create_note_wav(degree,name,left_frequency,right_frequency,duree) :
    if type(degree) != str :
        degree=str(degree)
    folder="Sounds/"
    filename=name+degree+".wav"
    sound=wave.open(folder+filename,'w') 
...
if __name__ == "__main__" :
    ...
    frequencies=cursor.execute("SELECT * FROM frequencies")
    notes=["notes","C","C#","D","D#","E","F","F#","G","G#","A","A#","B"]
    for frequency in frequencies :
         for i in range(1,len(frequency)) :
                        create_note_wav(frequency[0],notes[i],frequency[i],2*frequency[i],1)
    ##f0=440.0
